...
boxvalue=StringVar()
combobox = ttk.Combobox(app,textvariable=boxvalue,values=["-Select-","Dubai", "Mumbai", "Delhi",'Bangalore'])
combobox.grid(column=2,row=2)
combobox.current(0)
label1=Label(app,text='To',background='pink')
label1.grid(column=6,row=1)
boxvalue1=StringVar()
combobox1=ttk.Combobox(app,textvariable=boxvalue1,values=["-Select-","Dubai", "Mumbai", "Delhi",'Bangalore'])
combobox1.current(0)
...
db.commit()
def details_csv():
        query = pd.read_sql_query("select * from details",db)
        df = pd.DataFrame(query)
        df.to_csv('details.csv',index=False)
   
Label(app, text = "Method of Travel",background='pink').grid(row=6, column=4)
Button(app,text="Convert to csv",command = details_csv).grid(row=11,column=4)
...
Label(app,text="Class",background='pink').grid(row=8,column=2)

Radiobutton(app, text='Economy', variable = var1, value=1,command=cls).grid(row=9,column=2)
Radiobutton(app, text='Business', variable = var1, value=2,command=cls).grid(row=10,column=2)
...
tk.Button(app,text='Continue',command=end,bg='yellow').grid(column=6,row=10)
app.mainloop()
f1=boxvalue.get() 
f2=boxvalue1.get() 
sel1()    
...
def database():
    ...
    if adultnumber!=0:
        for i in range(adultnumber):
            mycursor=db.cursor()
            mycursor.execute("create table if not exists details(bnumber int,name varchar(20),passport varchar(20),type varchar(20),from1 varchar(20),to1 varchar(20))")
            db.commit()
    ...
    if childnumber!=0:
        for i in range(childnumber):
            ...
            db.commit()
    db.close()
    db = mysql.connector.connect(host='localhost',user='root',passwd='  ',database='airportmanagement')
    mycursor=db.cursor()
    mycursor.execute("use airportmanagement")
    mycursor.execute("create table if not exists payment(cardnumber varchar(20),cvv varchar(20),expmonth varchar(20),expyear varchar(20))")
...
def emirates1():     
    ...
    if f1=='Dubai' and f2=='Mumbai':
        ...
        if selection1=='2':  
            ...
            tprice=pricead1+pricech1
    elif f1=='Dubai' and f2=='Bangalore': 
        deptime1='1:30am'
        arrtime1='4:45am'
        ...
        if selection1=='2':  
            ...
            tprice=pricead1+pricech1
    elif f1=='Bangalore' and f2=='Dubai':  
        deptime1='6.00am'
        arrtime1='9:15am'
        totaltime='3 Hours and 15 Minutes'
        pricead1=110*adultnumber
...
def confirmation():
    ...
    Label(confirmation,text='Phone Number:312312341').grid(row=5,column=2)
    Label(confirmation,text='Email:admin@booksupport.com').grid(row=6,column=2)
...
def payments():#payment window
    ...
    img1=Image.open(r'C:\Users\Arya\Desktop\Schoolproject\final\card.jpg')
    ...
    Label(payments,text='>>Total amount to pay: dhs'+totalprice).grid(row=2,column=1)
...
def itenary(): #itenary and details window
    ...
    var3=IntVar()
    Checkbutton(itenary, text="Add Travel Insurance(dhs 15)",variable=var3,command=insure).grid(row=9,column=3)
    def func():
        itenary.destroy()
        details()
...
def details1():
    ...
    Label(itenary,text='Airline:'+flightname).grid(row=2,column=2)
    ...
    Label(itenary,text='Airline:'+flightname).grid(row=6,column=2)
    ...
    def outward():
        ...
        details()
    var8=IntVar()
    Checkbutton(itenary, text="Add Travel Insurance(dhs 15)",variable=var8,command=insure).grid(row=10,column=2) 
...
def oneway(): #oneway-gui
    ...
    if flag==1:#domestic
        ...
        Label(fselection,text='Airline      departure-arrival     ').grid(row=7,column=3)
        Label(fselection,text=flname1+'      '+deptime2+'      '+arrtime2+'  '+'dhs'+tprice1).grid(row=8,column=3)
        ...
        Label(fselection,text='Airline      departure-arrival     ').grid(row=3,column=3) 
        Label(fselection,text=flname+'      '+deptime1+'    '+arrtime1+'   '+'dhs'+tprice).grid(row=4,column=3)
...
def roundtrip():#roundtrip-GUI
    ...
    if flag==1:#domestic
        ...
        Label(round,text='Airline      departure--arrival     ').grid(row=3,column=2)
        ...
        Label(round,text='Airline      departure--arrival     ').grid(row=7,column=2)
        Label(round,text=flname1+'      '+deptime2+'      '+arrtime2).grid(row=8,column=2)
        Label(round,text='TotalPrice dhs:'+str(tprice1*2)).grid(row=9,column=2)
        f2,f1=f1,f2
        ...
        Label(round,text='Total Price: dhs '+str(tprice*2)).grid(row=9,column=2)
        ...
        Label(round,text='Airline      departure--arrival     ').grid(row=13,column=2)
        ...
        Label(round,text='Airline      departure--arrival     ').grid(row=17,column=2)
        Label(round,text=flname1+'      '+deptime2+'      '+arrtime2).grid(row=18,column=2)
        Label(round,text='Total Price: dhs '+str(tprice1*2)).grid(row=19,column=2)
        Button(round,text='Book',command=details1).grid(row=20,column=3)
        f2,f1=f1,f2
    round.mainloop()
if selection=='1':
    fselection=tk.Tk()
    fselection.title('OOB-Flightdetails-oneway') 
